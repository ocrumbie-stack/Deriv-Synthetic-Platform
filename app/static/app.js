...
function renderSymbolLeverage(rows) {
  const el = document.querySelector("#symbolLeverage");
  if (!el) return;
  if (!rows.length) { el.innerHTML = emptyRow(3, "No synthetic index leverage data available."); return; }
  el.innerHTML = rows.map(r => `
    <tr>
